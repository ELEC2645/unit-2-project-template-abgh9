// Electronics Helper - Main Menu (based on ELEC2645 template)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
static void select_menu_item(int input)
{
    switch (input) {
        case 1:
            ...
            break;
        case 2:
            menu_item_2(); /* Series/Parallel */
            go_back_to_main();
            break;
        case 3:
            menu_item_3(); /* Ohm's law */
            go_back_to_main();
            break;
        case 4:
            menu_item_4(); /* View history */
            go_back_to_main();
            break;
        default:
            printf("Bye!\n");
...
/* Return 1 if s is an optional [+/-] followed by digits */
static int is_integer(const char *s)
{
    if (!s || !*s) return 0;
    if (*s == '+' || *s == '-') s++;
